functor MkBSTTable
  ...
  :> ORDTABLE where type Key.t = BST.Key.t and type 'a Seq.t = 'a Seq.t =
struct
  ...
  struct
    ...
    type set = unit Base.t
    type t = set

    val size = Base.size
    val toSeq = Seq.map (fn (k,()) => k) o Base.toSeq
    fun toString s =
    ...
    fun singleton k = Base.$ (k, ())
    val $ = singleton
    val fromSeq = Base.fromSeq o Seq.map (fn k => (k, ()))

    fun find s k = Option.isSome (Base.find s k)
    fun insert (s, k) = Base.insert (s, (k, ()))
    val delete = Base.delete

    fun filterKey p = Base.filterKey (fn (k,()) => p k)

    fun reduceKey f b =
      #1 o Base.reduceKey (fn ((k1,_), (k2,_)) => (f (k1, k2), ())) (b,())
    fun iterateKey f b = Base.iterateKey (fn (x,(k,_)) => f (x,k)) b

    val union = Base.union (fn _ => ())
    fun intersection (s1, s2) = Base.intersection (fn _ => ()) (s1, s2)
  ...
  val size = Base.size
  fun domain t = Base.map (fn _ => ()) t
  fun range t = Seq.map (fn (_,v) => v) (Base.toSeq t)
  fun toString vtos t =
    let fun kvtos (k, v) = "(" ^ Key.toString k ^ " -> " ^ vtos v ^ ")"
  ...
  val $ = singleton
  fun tabulate f = Base.mapKey (fn (k,()) => f k)
  ...
  fun restrict (t, s) = intersection (fn (x,()) => x) (t, s)
