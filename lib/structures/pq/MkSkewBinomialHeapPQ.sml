functor MkSkewBinomialHeapPQ (structure Key : ORDKEY)
...
struct
  ...
  fun link (t1 as Node {data=x1, extra=xs1, children=c1},
      ...
      case compare (x1, x2) of
        ...
        | _       => Node {data=x1, extra=xs1, children=t2::c1}
  ...
  fun skewLink x (t1, t2) =
      ...
      in case compare (x, y) of
            ...
            | _       => Node {data=x, extra=y::ys, children=c}
      end
  ...
  fun insert ({trees=ts, min=m, size=s}, x) =
      {trees=case ts of
                 t1::t2::ts' =>
                 if rank t1 = rank t2
  ...
  fun insertList [] ts = ts
    | insertList (x::xs) ts = insertList xs (insert (ts, x))
