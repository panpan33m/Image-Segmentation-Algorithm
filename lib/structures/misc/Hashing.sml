...
struct
  ...
  (* a safe (rounded) toInt that works for any Int *)
  fun toIntRounded (w : word) =
    case Int.precision of
         NONE => toInt w
       | SOME intPrec =>
           (* Int.maxInt is 2^(intPrec-1)-1.
            * The maximum word is 2^wordSize-1. *)
           case Int.compare (Int.-(intPrec,1), wordSize) of
                GREATER => toInt w
              | EQUAL => toInt w
              | LESS => toInt (w mod (<< (0w1, Word.fromInt (Int.-(intPrec,1)))))
