...
functor MkChunkedTreeSequence (val grainsize : int) :> SEQUENCE =
struct
  open Primitives
  exception NYI

  structure BaseSeq : BASE_SEQUENCE = VectorBaseSequence
  ...
  fun map f S =
    ...
    | T2 (_,_,L,R) => makeT2 (par (fn () => map f L, fn () => map f R))
  ...
  fun cut S i =
    case S of
      T0 => (T0,T0)
    | T1 c => (T1 (BaseSeq.take c i), T1 (BaseSeq.drop c i))
    | T2 (_,_,L,R) =>
        let val l = length L
        in
          if (i = l) then (L,R)
  ...
  fun sort cmp s =
    ...
    in
      ...
      then s
        ...
        merge cmp (par (fn () => sort cmp (take s (n div 2)),
  ...
  fun zipWithEqualLen f (s1, s2) =
    case s1 of
      T0 => T0
    | T1 c => T1 (BaseSeq.tabulate (fn i => f (nth s1 i, nth s2 i)) (BaseSeq.length c))
    | T2 (_, _, l1, r1) =>
        let val (l2, r2) = cut s2 (length l1)
        in makeT2 (par (fn () => zipWithEqualLen f (l1, l2),
                        fn () => zipWithEqualLen f (r1, r2)))
        end

  fun zipWith f (s, t) =
    let val n = Int.min (length s, length t)
    in zipWithEqualLen f (take s n, take t n)
    end

  fun unzipWith f s = let val s' = map f s in (map #1 s', map #2 s') end
  ...
  fun reduce f b s =
    case s of
      T0 => b
    | T1 c => BaseSeq.iterate f b c
    | T2 (_, _, l, r) => f (par (fn () => reduce f b l, fn () => reduce f b r))
...
structure ChunkedTreeSequence = MkChunkedTreeSequence (val grainsize = 512)
