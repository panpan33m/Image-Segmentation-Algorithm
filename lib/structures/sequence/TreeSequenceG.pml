...
struct
  ...
  fun iterate g f b s = snd (iteratePrefixes g f b s)
  ...
  fun mapIdx g f = compose (map g f) (enum g)
  ...
  fun unzipWith g (spl : 'a -> 'b * 'c) s =
      let
        val n = length s
        val s' = map g spl s
      in (tabulate g (compose fst (nth s')) n, tabulate g (compose snd (nth s')) n)
      end
  ...
  fun filterIdx g p =
      compose (compose (map g (fn (_, x) => x)) (filter g p)) (enum g)

  fun equal g (cmp : 'a * 'a -> bool) (s1,s2) =
  ...
  val inject : int -> 'a seq * (int * 'a) seq -> 'a seq = inject
  ...
  fun collect g cmp s =
      let
        ...
        val lengths = zipWith g (fn (x,y) => x - y) (drop g (append g (starts, fromList [n])) 1, starts)

        fun make (i, len) = (nth ks i, subseq g vs (i, len))
      in zipWith g make (starts, lengths)
      end

  fun collate g (cmp : 'a * 'a -> order) (s1, s2) =
